...
// Import model code dynamically by copying it or reproducing it
// We'll reproduce the featurize and predict function in JS here
...
// Let's load model.ts via import or train it here
// We'll train a quick ensemble on the records using the same logic
const records = [];
...
// We need candidate split thresholds helper
...
// Test predict with different inputs
function predict(input) {
  const fv = featurize(input, enc);
  const treePredictions = trees.map(tree => predictTree(tree, fv) * 100);
  const impact = Math.round(mean(treePredictions));
  return { impact, treePredictions: treePredictions.slice(0, 5) };
}

const inputs = [
  { cause: 'accident', corridor: 'Tumkur Road', zone: 'Peenya', priority: 'Medium', eventType: 'unplanned', requiresClosure: false, hour: 14, durationMin: 60 },
  { cause: 'accident', corridor: 'Mysore Road', zone: 'Peenya', priority: 'Medium', eventType: 'unplanned', requiresClosure: false, hour: 14, durationMin: 60 },
  { cause: 'accident', corridor: 'Non-corridor', zone: 'Peenya', priority: 'Medium', eventType: 'unplanned', requiresClosure: false, hour: 14, durationMin: 60 },
  { cause: 'accident', corridor: 'Tumkur Road', zone: 'Central Zone 2', priority: 'Medium', eventType: 'unplanned', requiresClosure: false, hour: 14, durationMin: 60 },
  { cause: 'accident', corridor: 'Tumkur Road', zone: 'Jayanagara', priority: 'Medium', eventType: 'unplanned', requiresClosure: false, hour: 14, durationMin: 60 },
];

for (const ip of inputs) {
  console.log('Input Corridor:', ip.corridor, '| Zone:', ip.zone);
  console.log('Result Impact:', predict(ip).impact);
}
