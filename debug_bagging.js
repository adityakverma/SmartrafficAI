...
// Build tree with Feature Bagging (Random Forest split behavior)
function buildTree(X, y, indices, depth, maxDepth, minSamplesSplit, candidates) {
  ...
  // Randomly select a subset of features to split on (Feature Bagging)
  const allFeatureIndices = Array.from({ length: numFeatures }, (_, i) => i);
  // shuffle
  for (let i = allFeatureIndices.length - 1; i > 0; i--) {
    const r = Math.floor(Math.random() * (i + 1));
    [allFeatureIndices[i], allFeatureIndices[r]] = [allFeatureIndices[r], allFeatureIndices[i]];
  }
  // take subset (e.g. sqrt or half)
  const numSubset = Math.max(3, Math.floor(Math.sqrt(numFeatures)) + 1);
  const subsetFeatureIndices = allFeatureIndices.slice(0, numSubset);

  for (const j of subsetFeatureIndices) {
...
console.log('Base prediction (bagging):', predict(baseInput));
